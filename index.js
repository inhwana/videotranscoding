      ...
      async function bootstrap() {
        ...
        app.post("/transcode", verifyToken, async (req, res) => {
          ...
          try {
            ...
            res
              .status(500)
              .json({ error: `Transcoding failed: ${err.message}` });
          }
        });
        ...
        const checkIfAudioInS3 = async (videoId, userSub) => {
          ...
          const audioKey = `audio/${storedFileName.replace(
            /\.[^/.]+$/,
            ".mp3"
          )}`;
          ...
          try {
            ...
            console.warn(
              "updateVideoStatus(audio_ready) failed:",
              e.message || e
            );
          }
        ...
        app.post("/remove-audio", verifyToken, async (req, res) => {
          ...
          try {
            ...
            const presigned = await S3Presigner.getSignedUrl(
              s3Client,
              command,
              {
                expiresIn: 3600,
            ...
            res
              .status(500)
              .json({ error: err.message || "Audio extraction failed" });
          }
        });
